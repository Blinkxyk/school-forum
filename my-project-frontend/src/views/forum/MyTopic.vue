...
import { Clock, CircleCheck, Star } from "@element-plus/icons-vue";
...
onMounted(() => {
  get(`/api/forum/my-topic?uid=${store.user.id}`, (data) => {
  ...
  <div style="display: flex; margin: 20px auto; gap: 20px; max-width: 900px">
    <div style="flex: 1">
      <transition name="el-fade-in" mode="out-in">
        <div v-if="myTopics.length">
          <div style="margin-top: 10px; display: flex; flex-direction: column; gap: 10px">
            <light-card v-for="item in myTopics" :key="item.id" class="topic-card"
